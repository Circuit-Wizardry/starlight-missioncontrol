    ...
    for i in range(len(pins)):
        ...
        if pins[i].getTrigger() == 3: # altitude going UP
            if not apogee and pressure > pins[i].getCustom() and not pins[i].isTriggered():
                pins[i].trigger();
                timeouts.append([pins[i], pins[i].getFireLength()]);
        if pins[i].getTrigger() == 4: # ___ seconds after apogee
            pass;
        if pins[i].getTrigger() == 5: # at launch detected
            if accelY > 0.5 and not pins[i].isTriggered():
                pins[i].trigger();
                timeouts.append([pins[i], pins[i].getFireLength()]);
        if pins[i].getTrigger() == 6: # ___ seconds after launch detected
            pass;
        if pins[i].getTrigger() == 7: # at motor burnout
            pass;
        if pins[i].getTrigger() == 8: # ___ seconds after motor burnout
            pass;
        if pins[i].getTrigger() == 9: # at landing
            if apogee and pressure < 10:
                pins[i].trigger();
                timeouts.append([pins[i], pins[i].getFireLength()]);
        if pins[i].getTrigger() == 10: # ___ seconds after landing
            pass;
